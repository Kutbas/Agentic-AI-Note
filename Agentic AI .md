...
在下一个视频中，我们将探讨在 Agentic 工作流中，如何优雅地进行成本控制和速度优化。敬请期待！

### 延迟与成本优化

在架构和开发 Agentic 工作流时，很多工程师往往在一开始就陷入了“过早优化”的陷阱：纠结于每次 API 调用的几美分成本，或是为了省下几百毫秒而苦熬底层并发代码。

但来自顶尖 AI 团队的实战经验告诉我们：**永远把“输出质量（Quality）”放在第一位，把延迟和成本优化放到最后。**

构建出能够惊艳用户的高质量 Agent 才是最难的一环。当你的系统由于效果太好、吸引了海量用户，导致 API 账单爆炸时——恭喜你，这是一个“幸福的烦恼”。此时，再去动用工程手段进行降本增效，才是最合理的节奏。

当那一天真正到来时，我们该如何系统性地优化 Agent 的延迟与成本呢？秘诀就在于：**打破黑盒，进行精细化的基准测试（Benchmarking）。**

**1. 延迟优化 (Latency Optimization)：寻找时间黑洞**

Agentic 工作流通常是一个漫长的串行管线，为了缩短用户的整体等待时间，我们必须像外科医生一样，剖开整个链路，记录下每个组件的执行耗时（Run time）。

以深度研究智能体（Research Agent）为例，一段典型的耗时追踪（Trace）可能长这样：
1.  LLM 生成搜索词：**7秒**
2.  调用 Web Search 引擎：**5秒**
3.  LLM 筛选 5 个最佳信息源：**3秒**
4.  执行 Web Fetch（抓取网页）：**11秒**
5.  LLM 撰写最终长文报告：**18秒**

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260416201351321.png)

将这组数据摆在面前，优化的抓手立刻浮出水面：

**抓手一：引入并发机制 (Parallelism)**
你看，第 4 步“抓取 5 个网页”竟然耗时 11 秒。如果你的代码目前是顺序执行（一个个下载），那么将这段逻辑改为异步并发（如 Python 的 `asyncio.gather`），这 11 秒极有可能被压缩到 3 秒以内（取决于最慢的那个网站）。

**抓手二：模型降级与路由 (Model Routing)**
LLM 生成搜索词（第 1 步）竟然花了 7 秒！我们需要反思：想几个搜索词，真的需要动用推理速度最慢、最顶级的旗舰大模型吗？
我们完全可以尝试使用更小、更快的模型（如 Llama 3 8B 或是专用的轻量级模型）来执行这个相对简单的任务。此外，市面上不同的云服务商由于底层推理硬件（如专用的推理芯片）的差异，即使是同一个开源模型，吐字速度（Tokens per second）也有天壤之别。通过切换更快的供应商，你能轻易抠出几秒的延迟。

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260416204621832.png)

**2. 成本优化 (Cost Optimization)：算清每一笔经济账**

在商业化落地中，算账是一项核心能力。与测算时间类似，我们需要给工作流的每一个节点贴上“价格标签”。

Agent 的开销通常分为三类：
*   **LLM 节点**：按输入/输出的 Token 数量计费。
*   **工具与 API 节点**：按调用次数（Pay per API call）计费，如特定商业数据库或搜索引擎接口。
*   **计算节点**：如果你的 Agent 需要运行代码沙盒或专用的 ML 模型，会产生基于服务器容量/时长的计算成本。

我们再来看看这个 Research Agent 的“账单拆解”：
1.  LLM 生成搜索词：`$0.0004` (Tokens)
2.  Web Search 引擎调用：**`$0.016` (API call)**
3.  LLM 筛选信息源：`$0.0004` (Tokens)
4.  Web Fetch 与 PDF 解析工具：**`$0.032` (API call: $0.002 + PDF解析: $0.03)**
5.  LLM 撰写最终报告：`$0.0009` (Tokens)

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260416205502887.png)

**算账的震撼发现：**
如果不做细粒度的基准测试，你可能会本能地认为“大模型最烧钱，我要想办法减少 LLM 的调用”。
但这份账单无情地打破了直觉：**在这个特定的工作流中，最昂贵的根本不是大模型（几步加起来不到 `$0.002`），而是那几个外部工具 API！** 尤其是第三方的 PDF 解析工具，单次调用就吃掉了 `$0.03`，占了整体成本的绝对大头。

有了这个清晰的成本热力图，你的优化策略就变得极其明确了：
*   不要去折腾那几厘钱的大模型 Token 成本了。
*   立刻去市面上寻找更便宜的 PDF 解析开源方案（比如本地部署的轻量级解析包）。
*   审查 Web Search API 的套餐包，看看是否有更便宜的替代品。

**3. 总结**

无论是优化延迟还是成本，核心方法论是一致的：**停止盲目猜测，建立基准测试（Benchmarking），用冷酷的数据揪出那条隐藏的“长尾巴”或“吞金兽”。**

当你在工程实践中做到了：用 Error Analysis 提升了质量，用并发降低了延迟，用精细算账打平了商业模式的 ROI，你的 Agentic AI 应用就真正具备了走向生产环境、服务海量用户的成熟度。

至此，关于 Agent 系统构建的核心理论与优化哲学已经全部讲授完毕。在下一个模块的总结中，我们将回顾这段激动人心的旅程，并展望 Agentic AI 的未来发展脉络。

### “构建与分析”循环

在探讨了任务拆解、反思机制、工具调用、错误分析以及延迟/成本优化之后，我们已经掌握了构建 Agentic AI 系统的所有核心武器。

但是，拥有了这些武器，并不意味着你能立刻组装出一台完美的机器。在实际开发中，Agentic 系统的演进从来不是一条从 A 点到 B 点的直线，而是一个在两个核心动作之间不断来回摇摆的**钟摆过程**。

这两个核心动作就是：**构建（Build）** 与 **分析（Analyze）**。

**1. 开发范式的底层逻辑：双引擎驱动**

在主导或观察了大量 Agentic 项目后，行业内形成了一个共识：**区分平庸团队与顶尖团队的最核心标志，在于他们在这两个活动上的时间分配。**

缺乏经验的开发者往往会将 90% 的时间倾注在“构建（Build）”上——他们不知疲倦地重写代码、修改 Prompt、堆砌新工具，却很少停下来思考。结果往往是越改越乱，系统性能停滞不前。

而真正成熟的架构师深知，“分析（Analyze）”虽然看起来不像写代码那样“有建设性”，但它却是决定下一步“构建”方向的唯一导航仪。

让我们来看看这个 `Build <----> Analyze` 的双引擎是如何在系统生命周期的不同阶段驱动项目演进的：

**(1) 第一阶段：原型期 —— 敏捷构建与直觉诊断**

*   **Build**：不要陷入无休止的理论推演。迅速用代码拉起一个“粗糙但能跑通（Quick and dirty）”的端到端系统。
*   **Analyze**：在这个阶段，你不需要复杂的指标。直接**审查输出结果（Examine outputs）**，并翻阅底层的**执行轨迹（Traces）**。通过肉眼观察，建立起对系统哪些环节聪明、哪些环节愚蠢的初步“直觉（Gut sense）”。基于这种直觉，你再回到 Build 环节，对个别组件进行初步微调。

**(2) 第二阶段：成长期 —— 引入量化基准**

*   **Analyze**：当系统稍具雏形，直觉就不再可靠了。此时必须停下写业务代码的手，转而**构建基础评估集（Build evals）**。哪怕只有 10-20 个测试样例，只要能**计算出客观指标（Compute metrics）**，你就拥有了衡量系统端到端表现的尺子。
*   **Build**：带着这把尺子，你才能放心地去重构端到端的系统链路，或者深入打磨某个具体的 LLM 组件，因为指标会告诉你修改是对是错。

**(3) 第三阶段：成熟期 —— 精准切片与隔离调优**

*   **Analyze**：随着系统越来越庞大，端到端的指标已经无法告诉你“病灶”在哪了。此时，分析工作必须升级为严谨的**错误分析（Error analysis）**。通过建立统计表格，精准算出是哪个组件导致了最多的失败。针对该组件，进一步建立隔离的**组件级评估（Component-level evals）**。
*   **Build**：在极其纯粹的组件级 Eval 保护下，团队可以心无旁骛地对那个拖后腿的组件进行高频的算法替换、超参数调优甚至模型微调。

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260416210103262.png)

**2. 代码视角：如何将“分析”工程化？**

虽然市面上有许多优秀的第三方可观测性工具（Observability Tools）可以帮我们记录 Traces、监控运行时间和计算 Token 成本。但由于 Agentic 工作流往往携带着极强的定制化业务逻辑，**手写契合自身业务的自定义 Evals（Custom Evals）依然是高阶开发者的必修课。**

我们可以用一段高阶伪代码，来具象化这个贯穿开发始终的 `Build ↔ Analyze` 闭环框架：

```python
# 伪代码：Agentic 系统的自定义分析与评估引擎

class AgenticEvaluationEngine:
    def __init__(self, agent_workflow, test_dataset):
        self.agent = agent_workflow
        self.dataset = test_dataset
        self.error_ledger = {"retriever": 0, "planner": 0, "coder": 0}

    def run_analysis_loop(self):
        """执行端到端测试，并捕获 Trace 进行诊断"""
        for case in self.dataset:
            # 1. 执行并捕获完整的执行轨迹 (Traces)
            output, traces = self.agent.execute_with_traces(case.input)
            
            # 2. 端到端评估 (Compute metrics)
            if not self._evaluate_end_to_end(output, case.ground_truth):
                # 3. 触发错误分析 (Error Analysis)
                failed_component = self._diagnose_trace(traces)
                self.error_ledger[failed_component] += 1
                
        return self._generate_priority_report()

    def _diagnose_trace(self, traces):
        """深入 Trace 内部，定位并返回表现不佳的组件 (Span)"""
        # ... 复杂的业务诊断逻辑 ...
        return "planner" # 假设定位到是 Planner 环节出了问题
        
# 开发者根据 Engine 跑出的 priority_report，决定明天是去改 Retriever 的代码，还是改 Planner 的 Prompt。
```

在这套逻辑下，“分析”不再是虚无缥缈的复盘，而是实实在在的、能够输出指导性策略的代码工程。

**3. 模块结语与未来展望**

至此，关于“如何脚踏实地构建 Agentic AI”的实用技巧模块（Module 4）已经全部结束。如果你在未来的项目中，能够将本模块中哪怕一小部分的思想（比如：先用 Error Analysis 定位问题，再写 Component-level eval 隔离调优）付诸实践，**你的系统架构能力就已经超越了市面上绝大多数的 AI 开发者。**

现在，我们已经夯实了构建 Agent 的所有底层基础设施与评估方法论。系统已经做好了迎接更高智力挑战的准备。

在下一个、也是本系列课程的最后一个模块中，我们将向 AI 技术的深水区进发：**探讨那些更为前沿、复杂，且赋予大模型极高自主权的“高级设计模式（Advanced Design Patterns）”。** 准备好迎接高度自治的 AI 智能体了吗？我们最终章见！

## 高度自主的 Agent 模式

在过去的模块中，我们学习了如何通过硬编码的规则（Hard-coded rules）来引导 LLM 执行任务，也探讨了如何让 LLM 在特定的节点使用单一工具。

但是，面对真实世界中复杂、多变且充满不确定性的用户需求，如果我们试图为每一种可能的需求都提前写死一段执行逻辑，这不仅在工程上是不可能的，也会彻底扼杀 AI 的潜力。

为了构建**高度自治的智能体（Highly Autonomous Agents）**，我们必须引入一种高级设计模式：**规划模式（Planning）**。
在这种模式下，我们不再告诉 AI“怎么做”，而是给它一堆工具，告诉它“目标是什么”，让它自己去“写剧本”并分步执行。

**1. 场景一：全能的智能零售客服 Agent**

假设你经营一家眼镜店，底层有一个商品库存数据库（包含 ID、名称、描述、价格和库存数量）。

现在，一个极其挑剔的顾客抛出了一个复合型请求：
> *"你们有圆形的太阳镜吗？要有货，而且价格必须在 100 美元以内。"*

这是一个非常复杂的 Query，人类客服需要进行三步逻辑思考才能回答：
1. 先筛选出哪些款式是“圆形的”。
2. 再看看这些圆形款式里哪些“有库存”。
3. 最后看看这些有库存的款式里，哪些价格“低于 100 美元”。

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260416211110841.png)

如果用传统的编程思维，我们需要写极其复杂的 SQL 联表查询。但在 Agentic 规划模式下，我们只需要向大模型提供一个工具箱（包含：`获取商品描述`、`检查库存`、`处理退货`、`获取价格`、`查询历史交易` 等）。

然后，我们使用一个极具魔力的 **System Prompt（系统提示词）**：
> *"You have access to the following tools: {description of tools}. Return a step-by-step plan to carry out the user's request."*
> *(“你有权限访问以下工具：{工具描述}。请针对用户的请求，返回一个分步执行计划。”)*

接到指令后，LLM 会发挥其强大的推理能力，自主拆解任务，输出如下的 **Plan（执行计划）**：

1.  *Use `get_item_descriptions` tool to find round sunglasses.* (使用“获取描述”工具，找出圆形太阳镜)
2.  *Use `check_inventory` to see if results are in stock.* (使用“检查库存”工具，看看这些眼镜是否有货)
3.  *Use `get_item_price` to see if in-stock results are < $100.* (使用“获取价格”工具，筛选出低于 100 美元的款式)

**执行循环（Execution Loop）：**
有了计划后，Agent 的引擎开始运转。它将“步骤 1 的指令文本”和背景上下文喂给 LLM，触发工具调用并得到结果；接着，它带着“步骤 1 的结果”和“步骤 2 的指令文本”再次请求 LLM……如此循环，直到 3 个步骤全部走完，最终输出完美答案：“*是的，我们有一款经典的圆形金属框太阳镜，售价 60 美元，目前有货。*”

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260416211841933.png)

**规划的魅力在于它的普适性。**
如果顾客换了个要求：“*我想退掉之前买的金框眼镜，但保留金属框的。*”
Agent 会基于同样的工具箱，瞬间生成一个截然不同的新计划：先查历史订单 -> 再查哪个是金框 -> 最后调用退货接口。你作为开发者，**连一行硬编码的逻辑都不用改**！

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260416212121340.png)

**2. 场景二：智能邮件助理 (Email Assistant)**

让我们再看一个生产力场景。你对你的 AI 助理下达语音指令：
> *"回复 Bob 关于在纽约吃晚饭的那封邀请邮件，告诉他我会参加。然后把他的邮件归档。"*

系统底层只提供了 4 个基础工具：`搜邮件`、`移动邮件`、`删邮件`、`发邮件`。

面对这句极其随意的口语指令，Planning Agent 能够瞬间生成极其清晰的执行路线图：
1.  **检索**：使用 `search_email`，过滤发件人 "Bob" 以及关键词 "dinner" 和 "New York"。
2.  **动作 1**：基于检索到的上下文，使用 `send_email` 工具撰写并发送一封确认出席的回信。
3.  **动作 2**：确认发送成功后，使用 `move_email` 工具将原邮件移至 "archive"（归档）文件夹。

在引擎的调度下，LLM 像一个严谨的秘书一样，一步步完成了这个跨越多个工具状态机的复杂任务。

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260416212346035.png)

**3. 规划模式的现状与挑战**

目前，这种高度自治的 Planning 模式已经在**自动化软件开发（Agentic Coding Systems）**领域取得了巨大的商业成功（如 Devin 等产品）。当你要求 AI“写一个小游戏”时，它能自己列出一个长长的 TODO 清单：搭框架 -> 写前端 -> 写后端逻辑 -> 跑测试，然后按部就班地把代码写出来。

然而，在其他容错率较低的商业领域，Planning 模式的应用依然偏向实验性质。
其面临的**核心挑战在于：不可控性（Hard to control）。**
作为开发者，你无法在代码运行前预知 LLM 到底会制定出怎样天马行空的计划。如果它在第一步规划时就发生了幻觉，或者在中间某一步走入了死胡同，整个工作流就会彻底崩溃。

**4. 总结与预告**

规划模式（Planning）是通往 AGI（通用人工智能）的重要里程碑，它让 AI 从“被动接受硬指令的工具”进化为了“能够自主思考路径的解决者”。

但在上一节中，我们仅仅是在一个极高的抽象层面上展示了“计划列表”和“分步执行”。在真正的底层代码中，这种“自我规划与执行”的引擎究竟是如何运作的？当步骤执行失败时，Agent 又是如何进行调整的？

在下一节的深度剖析中，我们将带你走进 Planning Agent 的核心引擎，去看看那些字符串是如何交织成一套完整的执行流的。敬请期待！

### 让 Agent 生成机器可读的执行计划

在上一节中，我们见识了 Agentic 规划模式（Planning）的强大威力。大模型能够自主生成如“第一步找圆形眼镜，第二步查库存，第三步查价格”这样清晰的思维链。

然而，在实际的软件工程中，这里存在一个巨大的断层：**虽然这段纯文本计划（Plain Text）人类看着很直观，但后端的调度代码（Orchestrator）根本无法解析它。**

后端代码无法直接读懂“*Use `get_item_descriptions` tool*”这句话，它需要知道确切的函数名、参数类型以及参数值。如果完全依赖自然语言去驱动系统，必然会充满歧义（Ambiguous）且极不稳定。

为了让高度自治的系统变得可靠，我们必须引入一种强制性的结构化契约。

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260417101923676.png)

**1. 核心解法：强制输出结构化 JSON**

在现代 Agent 开发中，架构师们通常会要求 LLM 将其思考出来的计划，严格按照 **JSON（JavaScript Object Notation）格式**进行输出。

因为当今所有主流的顶级大模型（如 GPT-4o、Claude 3.5），在经过海量代码数据的预训练后，生成标准 JSON 的能力都已经炉火纯青。JSON 键值对（Key-Value pairs）的严谨性，使得下游的 Python 调度代码能够像读取本地字典一样，毫无歧义地解析出每一步的意图和所需工具。

为了实现这一点，我们需要对系统提示词（System Prompt）进行一次极其严厉的升级：

> **Updated System prompt (升级版系统提示词):**
> *"You have access to the following tools: {description of tools}."*
> *"Create a step-by-step plan in **JSON format**."*
> *"Each step should have the following exact keys: `step` (number), `description` (string), `tool` (string), and `args` (object)."*
> *(“你有权限访问以下工具……请以 JSON 格式创建一个分步计划。每一个步骤都必须包含以下具体的键名：步骤编号、描述、工具名称、以及参数对象。”)*

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260417102314072.png)

**2. 机器视角的完美计划**

当用户再次抛出那个复杂的查询：“*你们有 100 美元以内、有货的圆形太阳镜吗？*”

接收到升级版 Prompt 的 LLM，不再输出闲聊式的自然语言，而是直接返回一个嵌套严密、逻辑严谨的 JSON 数组：
...
```json
{
  "plan": [
    ...
    },
    {
      ...
      "args": {
        "items": "results from step 1" 
      }
    },
    {
      "step": 3,
      "description": "Check if prices are under $100",
      "tool": "get_item_price",
      "args": {
        "items": "results from step 2"
      }
    }
  ]
}
...
**解析过程：**
当后端调度程序接收到这串 JSON 时，事情就变得极其简单了：
1.  **解析数组**：程序读取 `plan[0]`。
2.  **提取函数名与参数**：识别到 `tool` 的值是 `"get_item_descriptions"`，`args` 中包含了 `{"query": "round sunglasses"}`。
3.  **精确执行**：程序直接触发本地函数调用 `get_item_descriptions(query="round sunglasses")`，并将返回的结果作为上下文，准备注入到第二步中。

注意看第二步中的 `args` -> `"items": "results from step 1"`。这种基于占位符（Placeholder）的参数传递，是高级 Agent 规划中的常见策略，调度引擎会在运行时将上一步的真实返回值替换进去。

**3. 格式之争：JSON、XML 还是 Markdown？**

除了 JSON，业界还有一些其他的结构化输出选择，它们各有优劣：

*   **JSON (极力推荐)**：目前业界绝对的工业标准，所有的主流编程语言都有极速的解析库，且大模型对其语法的掌握最为精确。
*   **XML (备选方案)**：使用 `<step1> ... </step1>` 这样的标签来界定步骤。在某些特定模型（如部分版本的 Claude）上表现优异，解析起来也相对稳定。
*   **Markdown / 纯文本 (强烈不推荐)**：使用 `- Step 1:` 或者加粗文本来表示。这种格式极其依赖正则表达式来提取信息，一旦大模型多输出了一个空格或者改变了标点符号，提取代码就可能全面崩溃。

**4. 总结与展望**

通过强制模型输出 JSON 格式，我们将天马行空的 LLM 规划能力，成功装入了一个严谨、可解析的工程化容器中。这使得“分步执行多工具计划”成为了一项稳定可靠的技术栈。

但这真的是表达复杂计划的终极方案吗？

如果我们想要表达极其复杂的控制流呢？比如：*“如果查不到圆形眼镜，就自动去查方形眼镜（If-Else 分支）”*，或者*“针对查到的每一副眼镜，循环查询它的库存（For Loop）”*。
使用 JSON 来表达复杂的条件分支和循环结构，将会变得极其冗长且痛苦。

有没有一种更简洁、更原生、表达能力无限的“语言”，能够让大模型完美地表达极其复杂的逻辑计划？
答案是肯定的：**让大模型直接写代码（Code as a Plan）！**

在下一节中，我们将探索这项令人惊叹的前沿技术：如何让大模型直接用 Python 代码来编写和执行整个计划。敬请期待！

### 用代码来执行计划

在前面的章节中，我们学习了如何通过强制大模型输出 JSON 格式（JSON Formatting），来实现机器可读的执行计划。

但是，当你把这种 JSON 规划引擎部署到真实的业务场景中时，很快就会遇到一面极其厚重的墙——**工具爆炸与边缘情况（Tool Explosion & Edge Cases）。**

**1. 传统 API 规划的困境：脆弱、低效且永远写不完的工具**

假设你开发了一个“数据分析 Agent”，专门用于解答关于咖啡店销量的各种问题（基于一个庞大的 `coffee_sales.csv` 表格）。
一开始，你非常勤奋地用 Python 写了一堆数据处理工具，并把它们封装成了 JSON Schema 供模型调用，比如：`get_column_max`（求最大值）、`filter_rows`（过滤行）、`sum_rows`（求和）等。

现在，用户问了一个稍微复杂点的问题：
> *“哪个月的热巧克力销量最高？”*

如果要用你的这些工具来完成，JSON 规划器需要生成一个极其冗长且愚蠢的计划：
1. 过滤 1 月份所有“热巧克力”的行。
2. 求 1 月份的销量平均值。
3. 过滤 2 月份的……求 2 月份的平均值。
4. ……重复这个过程直到 12 月。
5. 比较这 12 个数字，找出最大的那个月。

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260417104424179.png)

这种长串联的 JSON 调用不仅**效率极低（Inefficient）**，而且中间任何一步网络超时都会导致全盘崩溃，系统变得**极其脆弱（Brittle）**。

更糟糕的是，如果用户换个问法：“*上周有多少笔**独立的**交易？*”
你突然发现，你的工具箱里根本没有 `get_unique_entries` 这个工具！于是你只能跑回去改系统代码，新加一个工具。
然后用户又问：“*最近 5 笔交易的金额是多少？*”
你又要去加一个 `get_last_N_values` 工具。

你会陷入一个永远在**处理边缘情况（Continuously dealing with edge cases）**的死循环，最终你的代码库里堆积了成百上千个极其琐碎的工具 API。

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260417105128180.png)

**2. 破局之道：Code as Action（以代码代替 JSON 行动）**

与其费尽心机给大模型提供一堆封装好的“玩具 API”，为什么不直接把最高权限的底层武器交给它呢？

**这个底层武器就是：Python 编程语言和庞大的开源生态。**

我们可以彻底抛弃 JSON 规划器，转而使用一个极其硬核的 System Prompt：
> *"Write code to solve the user's query. Return your answer as python code delimited with `<execute_python>` and `</execute_python>` tags."*
> *(“请编写代码来解决用户的查询。请将答案作为 Python 代码返回，并使用 `<execute_python>` 标签包裹。”)*

当用户问：“*最近 5 笔交易的金额是多少？*”
LLM 不再去思考要调用哪个你写的蹩脚 API，它直接凭借预训练时吸收的海量程序员智慧，写下了一段极其优雅、自带执行逻辑的 Pandas 代码：

```python
<execute_python>
import pandas as pd

# 计划步骤 1: 读取 CSV 文件并解析日期
df = pd.read_csv("transactions.csv")
df["date"] = pd.to_datetime(df["date"])

# 计划步骤 2: 按日期倒序排列，获取最新交易
...
**为什么这种方式是降维打击？**
因为 Python 语言本身（加上 Pandas 这类数据处理库），就包含了成百上千个高度优化过的函数。**大模型在预训练时，已经看过几十亿遍这些函数的正确使用方法了！** 
通过写代码，LLM 相当于瞬间拥有了无限的工具。它能够在一个单一的代码块中，利用变量赋值、If-Else 条件分支、For 循环，把极其复杂的逻辑（计划）全部串联在一起，一次性扔给底层的沙盒去执行。

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260417110722097.png)

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260417130940671.png)

**3. 学术界的数据支持：代码行动全面碾压文本与 JSON**

这种架构设计的优势并非空穴来风。在学术界极具影响力的论文 *Executable Code Actions Elicit Better LLM Agents (Wang et al., 2024)* 中，研究人员横向对比了这三种 Agent 规划模式的成功率。

实验数据（涵盖了 GPT-4、Claude-2、Llama-2 等各种级别的模型）呈现出一个不可辩驳的阶梯式碾压趋势：
*   **Text as Action (纯文本指令)**：成功率垫底。
*   **JSON as Action (JSON 结构化指令)**：成功率居中，由于消除了歧义，比纯文本有显著提升。
*   **Code as Action (代码执行引擎)**：成功率登顶！即使是较弱的模型（如 GPT-3.5 或 Llama-2），在允许其写代码执行时，其任务完成率也远远超过了只能输出 JSON 的 GPT-4！

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260417131201393.png)

**4. 总结与展望**

利用“写代码并执行”来进行规划，目前已经在**高阶 AI 程序员（Agentic Software Coders，如 Devin、Cursor 等）**领域大放异彩。当你让它“写个小游戏”时，它能自己用代码列出一个复杂的 TODO List，然后挨个写代码去实现和测试。

当然，这也是一把双刃剑。正如我们在上一章强调的，**执行任意代码必须在一个安全的沙盒（Sandbox）中进行**。并且，由于你完全放弃了对底层步骤的控制，系统在运行时的状态会变得更加不可预测。

至此，关于单个 Agent 如何通过“规划（Planning）”实现高度自治的模式已经全部探讨完毕。

但在某些极其复杂的庞大工程面前，一个全能的 Agent 也会显得力不从心。如果一个项目需要深入调研、写出神级文案、还要经过严苛的代码测试，我们能指望一个模型包揽一切吗？
在接下来的最后一个视频中，我们将踏入 Agentic AI 最激动人心的新领域：**多智能体协作 (Multi-Agent Systems)**——看看如何组建一支 AI 的虚拟军队，让它们通过彼此协作来完成令人惊叹的壮举。敬请期待！

### 多智能体协作

随着我们掌握了如何让单个 Agent 使用工具、反思乃至自主规划执行路径，我们已经能够应对绝大多数复杂的个人级任务。

但是，真实世界中的大型工程（如准备一场诉讼、撰写一份详尽的商业研究报告、策划一场完整的营销活动），从来都不是靠一个人“单打独斗”完成的。即便一个人的能力再全面，在处理跨领域的复杂任务时，也会因为精力分散、缺乏专业视角的交叉验证而导致产出质量下降。

**这个道理，对人类适用，对大语言模型（LLM）同样适用。**

当我们把极其复杂的复合型任务硬塞给一个单一的 LLM 时，它很容易在多重指令的重压下发生崩溃或逻辑混乱。为了解决这个问题，行业内诞生了 Agentic 架构的终极形态：**多智能体协作（Multi-agent Workflows）**。

**1. 核心哲学：像设计组织架构一样设计系统**

许多初学者会对多智能体架构产生疑问：“既然底层调用的都是同一个大模型（比如都是调用 GPT-4 的 API），为什么非要搞出好几个 Agent 来相互对话？这不是多此一举吗？”

我们可以用计算机操作系统来做个类比。虽然你的电脑里只有一块 CPU，但作为程序员，你会把复杂的任务拆解成多个线程（Threads）或进程（Processes）来并发执行。这种拆解不仅让代码更易维护，也能大幅提升系统的稳定性和效率。

在设计多智能体系统时，我们不需要懂深奥的代码，我们只需要具备一种“HR（人力资源）的思维”：**不要去想“我要雇哪一个全能天才来帮我干活”，而是去想“为了干好这个活，我需要组建一个怎样角色的团队？”**

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260420095207424.png)

以“策划一场夏季太阳镜营销活动”为例。一个标准的营销团队通常包含三个角色。在 Agentic 架构中，我们可以分别为这三个角色实例化出三个独立的 Agent：

1.  **Researcher (研究员 Agent)**
    *   **职责**：分析市场趋势，调研竞品。
    *   **工具箱 (Tools)**：只配备 `Web Search` (网络搜索)。
    *   **Prompt 设定**：“你是一位严谨的市场研究员，你的任务是全网搜集最新趋势并提炼客观数据……”
2.  **Graphic Designer (设计师 Agent)**
    *   **职责**：创建数据可视化图表和宣传海报。
    *   **工具箱 (Tools)**：`Image Generation API` (绘图), `Code Execution` (写代码画图表)。
    *   **Prompt 设定**：“你是一位审美极佳的平面设计师，请根据市场数据生成吸引眼球的视觉资产……”
3.  **Writer (文案 Agent)**
    *   **职责**：将干瘪的研究数据和视觉素材，转化为极具煽动性的营销文案。
    *   **工具箱 (Tools)**：无需外部工具，纯靠 LLM 强大的文字生成能力。
    *   **Prompt 设定**：“你是一位顶级的广告文案专家……”

通过这种角色的拆解，我们可以让每个 Agent 专注于自己最擅长的事情。更美妙的是，**这种架构实现了代码的极度解耦**。负责“研究员”代码的工程师可以去死磕搜索准确率，负责“设计师”代码的工程师可以去死磕图像生成的质量，两者互不干扰。并且，这个成熟的“设计师 Agent”未来还可以被复用到其他需要画图的项目中去。

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260420095441592.png)

**2. 架构模式一：线性流水线 (Linear Plan)**

最基础的多智能体协作模式，是类似于工厂车间的**线性流水线（Linear Plan）**。

当用户下达指令：“*创建夏季太阳镜营销活动*”时：
1.  **研究员 Agent** 率先启动，生成一份详尽的市场洞察报告（*Here are current sunglasses trends...*）。
2.  这份报告被作为输入（Input），直接传递给**设计师 Agent**。设计师根据报告中的数据点，生成了 5 张图表和 5 张宣传海报。
3.  最后，（研究报告 + 视觉素材）被打包扔给**文案 Agent**。文案 Agent 综合所有素材，输出一份排版精美、图文并茂的最终版营销手册。

线性流水线的优势在于**极度稳定、易于控制**。整个流程按部就班，不会出现不可控的逻辑分岔。

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260420095819459.png)

**3. 架构模式二：引入管理者的动态规划 (Planning with multiple agents)**

线性流水线虽然稳定，但缺乏灵活性。如果任务比较复杂，可能需要研究员和设计师反复沟通几次才能定稿。

为了实现更高级的协作，我们可以将上一节学过的**规划模式（Planning）**与**多智能体（Multi-agent）**结合起来。此时，我们将工具（Tools）替换为了具体的 Agent，并引入了一个全新的角色：**营销经理（Marketing Manager Agent）**。

我们赋予这个“经理 Agent”如下的 System Prompt：
> *"You are a marketing manager and have the following team of agents to work with: {description of agents}. Return a step-by-step plan to carry out the user's request."*
> *(“你是一名营销经理，你手下有以下团队成员（研究员、设计师、文案）……请输出一个分步执行计划。”)*

接到用户的宏大指令后，**经理 Agent 并不亲自干活，而是负责“排兵布阵”**：
1.  *Ask researcher to research current sunglasses trends.* (派研究员去查资料)
2.  *Ask graphic designer to create ad images.* (派设计师去做图)
3.  *Ask writer to create report.* (派文案去写报告)
4.  *Review report.* (最后由我，经理本人，来进行审核和反思)

在执行阶段，调度引擎会将第一步的指令发送给研究员，拿到结果后传回给经理；经理再将第二步的指令连同结果发给设计师……以此类推。

在这个架构中，**经理 Agent 就像是整个虚拟公司的大脑（Orchestrator）**。它动态地协调着各个专职 Agent 的工作，甚至可以在最后一步进行全局的反思（Reflection），如果觉得文案写得不够好，经理有权把任务打回给文案 Agent 重新修改。

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260420100141208.png)

**4. 总结与展望**

从让 AI 充当一个全能的打字员，到组建一支拥有经理、研究员、设计师和文案的**虚拟 AI 军团**，Agentic 架构的演进彻底颠覆了我们对软件工程的认知。

在本文中，我们看到了线性的“流水线模式”和中心化的“管理者模式”。但这仅仅是冰山一角。在真实复杂的业务场景中，智能体之间的协作和沟通机制（Communication patterns）还有哪些更精彩的形态？如果两个 Agent 意见不合，它们该如何辩论？

在下一个、也是本课程的最后一个技术视频中，我们将带你全面盘点多智能体系统中那些最前沿、最具潜力的**网络拓扑结构（Communication Patterns）**。敬请期待！

### 多智能体的通信模式

在现实商业世界中，当你管理着一个由几十人组成的团队时，如何设计合理的组织架构（Organizational Chart）和汇报路线，直接决定了团队的产出效率和执行力。

在多智能体系统（Multi-agent Systems）中，情况完全一样。当我们实例化了多个不同角色的 Agent（如研究员、设计师、文案）后，**它们之间应该以何种网络拓扑结构进行沟通和数据传递？** 这成为了决定系统稳定性与智能涌现程度的关键设计决策。

目前，在 AI 架构设计领域，主要演进出了以下四种典型的通信模式。

**1. 线性通信模式 (Linear Pattern)：流水线式的高效协作**

这是目前工业界应用最广泛、最稳定的一种通信模式。它的逻辑极其简单粗暴：任务像流水线上的产品一样，单向向下游传递。

在“策划夏季营销活动”的案例中，通信链路是这样的：
**研究员 (Researcher) -> 设计师 (Graphic Designer) -> 文案 (Writer)**

*   **特点**：研究员做完调研后，将报告直接甩给设计师；设计师画完图，连同调研报告一起甩给文案；最终由文案负责输出定稿。
*   **优势**：极度稳定、易于调试。因为数据流向是单向且确定的，当最终结果出错时，很容易定位是哪一个环节出了问题。

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260420101612286.png)

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260420101800828.png)

**2. 星型/层次化通信模式 (Hierarchical Pattern)：中心化管理者的调度**

当任务变得复杂，需要反复横跳时，线性流水线就不够用了。此时，我们需要引入我们在上一节提到的“管理者 Agent（Marketing Manager）”，形成一个类似于星星形状的中心化拓扑结构。

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260420101951743.png)

通信链路变成了：

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260420102922638.png)

*   **特点**：底层员工（研究员、设计师、文案）之间**绝对不直接沟通**。所有的数据交换和任务指派，都必须通过位于中心的“经理 Agent”进行中转。经理拿到研究报告后，亲自将其发给设计师。
*   **优势**：这种“规划模式（Planning）”赋予了系统极高的灵活性。经理可以进行全局把控，如果觉得初稿不满意，随时可以再次唤醒某个底层 Agent 进行返工（Reflection）。

**3. 深度层级通信模式 (Deeper Hierarchy)：构建庞大的虚拟企业**

如果我们将星型模式进一步拓展，允许底层的 Agent 拥有自己的下属，我们就构建出了一个极其复杂的**深度层级网络（Deeper Hierarchy）**。

*   **Manager** 管理着三大主管（Researcher, Graphic Designer, Writer）。
*   **Researcher 主管** 手下管理着：`Web Researcher` (专门负责上网搜索) 和 `Fact Checker` (专门负责对查到的资料进行事实核查)。
*   **Writer 主管** 手下管理着：`Style Writer` (专门负责调整文风) 和 `Citation Checker` (专门负责核对引用格式)。

*   **现状**：这种模式完美复刻了大型跨国公司的科层制架构。虽然它在理论上能处理极其庞大且复杂的工程，但目前的工程实现难度极高，因此在当今的生产环境中较少被大规模使用。


![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260420102136945.png)

**4. 全连通通信模式 (All-to-all Pattern)：混沌中的智能涌现**

这或许是目前最前沿、最具实验性质，也最令人兴奋的一种通信模式。

在这个拓扑结构中，没有层级，没有经理，**每一个 Agent 都可以随时随地向任何一个其他的 Agent 发送消息。**

*   **实现机制**：在系统初始化时，我们会告诉所有的 Agent：“*这个聊天室里还有另外三个人，你们可以随时互相 `@`。*”当一个 Agent 发出消息后，这条消息会被自动同步到接收者的上下文（Context）中。接收者会自己思考，决定要不要立刻回复。
*   **运行状态**：系统启动后，就像一个极其喧闹的头脑风暴会议室。大家七嘴八舌地讨论、辩论、修改，直到某一个 Agent（比如 Writer）认为“好，我看讨论得差不多了，这版文案可以定稿了”，或者所有人都声明任务结束，系统才会输出最终结果。
*   **优势与风险**：全连通模式的结果具有极强的**不可预测性（Unpredictability）**。它有时候能涌现出令人惊叹的创意方案，但有时候也会陷入无休止的“扯皮”死循环。因此，它只适用于那些需要高度发散思维、对控制流要求不高的容错型应用（比如头脑风暴、写创意小说）。

**5. 总结：选择适合的通信框架**

设计多智能体的通信模式，本质上是在**“控制力（Control）”与“自主性（Autonomy）”之间寻找平衡点。**
如果你需要系统绝不出错，选择线性或星型模式；如果你愿意忍受一点点混乱来换取惊艳的智能涌现，不妨大胆尝试全连通模式。

幸运的是，随着 Agent 技术的爆发，如今市面上已经涌现出了大量优秀的**多智能体开发框架（如 AutoGen、CrewAI、LangGraph 等）**。这些框架在底层已经为你封装好了这四种通信模式的调度逻辑，你只需要通过几行简单的代码，就能让你的虚拟团队开始运转。

随着对这些高级网络拓扑结构的深入理解，你已经彻底掌握了当今 AI 领域最前沿的架构设计理念。接下来，让我们进入本次课程的最终章，由吴恩达教授为我们带来这场激动人心的技术旅程的全面总结。

## 总结

在这段深入 Agentic AI（代理化人工智能）的奇妙旅程中，我们系统性地拆解了这项正在重塑软件工程边界的前沿技术。

如果说过去一年大语言模型（LLM）的爆发，让我们看到了 AI 的“智力”；那么 Agentic 工作流的崛起，则让我们赋予了 AI 真正改变世界的“行动力”。

在课程的尾声，让我们重新梳理这份架构师的知识图谱，回顾我们是如何一步步将一个只会聊天的文本模型，武装成一个具备高度自治能力的超级智能体的。

**1. 核心知识体系图谱 (The Agentic AI Knowledge Map)**

**1. 为什么我们需要 Agentic AI？(Why Agentic AI)**
我们跳出了传统的“直接生成（Zero-shot Prompting）”思维陷阱。我们认识到，通过将复杂任务拆解，并引入并行处理机制，Agentic 工作流不仅能够打破基础模型的性能天花板，甚至能在某些场景下超越人类专家的数据处理极限。

**2. 反思设计模式 (Reflection Design Pattern)**
这是性价比最高的设计模式。通过让模型审视自身的初稿，特别是当模型能够接收到**外部真实反馈（External Feedback，如代码报错日志）**时，系统的性能和逻辑严谨度将获得惊人的跃升。

**3. 工具调用与函数执行 (Tool Use & Function Calling)**
我们解密了 LLM 是如何长出“手脚”的。从底层的 JSON Schema 语法，到强大的 API 调度，再到被称为“特权核武器”的**代码执行沙盒（Code Execution）**。此外，我们还拥抱了工业级标准 **MCP（Model Context Protocol）**，彻底打破了应用与海量数据源之间的孤岛。

**4. 评估与错误分析 (Evals & Error Analysis)**
这是区分“玩具开发者”与“顶尖架构师”的绝对分水岭。我们学习了如何构建 2x2 的评估矩阵，如何编写代码或使用 LLM 裁判（LLM-as-a-judge）进行打分。更重要的是，我们掌握了贯穿开发始终的 `Build ↔ Analyze` 双擎驱动法则：通过审查执行轨迹（Traces），进行科学的错误归因分析，将研发精力精准投入到最值得优化的组件上。

**5. 智能规划与多智能体系统 (Planning & Multi-agent Systems)**
我们迈入了 AI 架构的深水区。从让 LLM 自主生成机器可读的 JSON 执行步骤，到震撼的“以代码代行动（Code as Action）”；从简单的线性流水线，到构建拥有经理、研究员、设计师的复杂“AI 虚拟公司”。我们见证了高度自治 AI 系统的无限潜力与设计之美。

![](https://graphbed-1331926955.cos.ap-shanghai.myqcloud.com/GraphBed/20260420103353050.png)

**2. 职业发展与未来期许**

掌握了这套完整的 Agentic AI 武器库，你已经超越了绝大多数仍停留在“写提示词”阶段的 AI 开发者。

正如吴恩达（Andrew Ng）教授在结语中所提到的现实情况：**当顶尖科技公司和 AI 创业团队在面试候选人时，他们重点考察的核心能力，正是你在本课程中所学到的这些硬核工程技能**（如任务拆解、构建 Evals 体系、多 Agent 架构设计等）。

无论你是希望在职场上斩获更广阔的职业机遇，还是仅仅出于对技术纯粹的热爱，想自己动手打造改变生活习惯的酷炫软件，这套 Agentic 架构思维都将成为你最强有力的支点。

技术的发展永无止境，而你现在已经拿到了通往未来的门票。

告别概念的炒作，回归工程的本质。请带着这份严谨的架构师思维，负责任地使用这些技术，去构建那些真正令人惊叹的 AI 应用吧！
